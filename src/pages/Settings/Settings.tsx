...
type Category = 'jeu' | 'mods' | 'perf' | 'discord' | 'legal';
...
export const Settings: React.FC<SettingsProps> = ({ onClose }) => {
    ...
    const handleRestoreOriginBackup = useCallback(async () => {
        if (!gamePath) return;
        ...
        try {
            ...
            setOriginBackupError(e.message ?? 'Erreur');
        } finally {
            ...
            case 'jeu':
                return (
                    <div className={styles.tabContent}>
                        ...
                        <div className={styles.originBackupBox}>
                            <div className={styles.originBackupHeader}>
                                <span className={styles.originBackupLabel}>JAR d'origine</span>
                            </div>
                            <p className={styles.originBackupDesc}>
                                Remplace <code className={styles.code}>projectzomboid.jar</code> par le JAR original téléchargé depuis le dépôt (version saine, même si le .bak est absent ou corrompu).
                            </p>
                            {!gamePath ? (
                                <p className={styles.originBackupWarn}>Jeu non détecté — détectez le jeu depuis l'écran principal.</p>
                            ) : (
                                <>
                                    {originBackupError && <p className={styles.ramError}>{originBackupError}</p>}
                                    <motion.button
                                        type="button"
                                        className={`${styles.applyBtn} ${originBackupSuccess ? styles.applyBtnSaved : ''}`}
                                        onClick={handleRestoreOriginBackup}
                                        disabled={originBackupLoading}
                                        whileHover={!originBackupLoading ? { scale: 1.02 } : {}}
                                        whileTap={!originBackupLoading ? { scale: 0.97 } : {}}
                                    >
                                        {originBackupLoading ? 'Téléchargement…' : originBackupSuccess ? '✓ JAR restauré' : 'Restaurer le JAR d\'origine'}
                                    </motion.button>
                                </>
                            )}
                        </div>
